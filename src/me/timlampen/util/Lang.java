...
public class Lang {
	...
	public void doMessage(Player player, Error r, String args){
		...
		case NULL_PLAYER:
			...
			break;
		case NO_MONEY:
			s = ChatColor.translateAlternateColorCodes('&', p.getConfig().getString("Messages.NoMoney".replace("%number%", args), "&4Error: [Not Enough Money] Options: [/dc] [You need $" + args + "]"));
			//Does need args
			break;
		case NO_TOKEN:
			s = ChatColor.translateAlternateColorCodes('&', p.getConfig().getString("Messages.NoTokens".replace("%crystals%", args), "&4Error: [Not Enough Crystals] Options: [/dc] [You need " + args + " crystal(s)]"));
			//Does need args
			break;
		case COOLDOWN:
	...
	public void doMessage(Player player, Success r, String name, String num){
		...
		case SIGN_USE:
			...
			//add to config
		case TOKEN_GET:
			s = ChatColor.translateAlternateColorCodes('&', p.getConfig().getString("Messages.TokenGet".replace("%player%", name).replace("%number%", num), "&6You have been given " + num + " crystal(s) by " + name + "!"));
			break;
			//add to config
		case TOKEN_GIVE:
			s = ChatColor.translateAlternateColorCodes('&', p.getConfig().getString("Messages.TokenGive".replace("%player%", name).replace("%number%", num), "&6You have given " + name + " " + num + " crystal(s)!"));
			break;
			//add to config
		case BANK_WITHDRAW:
			s = ChatColor.translateAlternateColorCodes('&', p.getConfig().getString("Messages.BankWithdraw".replace("%number%", num), "&6You just wrote a check for " + num + "! The money has been withdrawn"));
			break;
			//add to config
		default:
			break;
	...
	public void doMessage(CommandSender player, Mutual m, String name, String num){
		String s = null;
		switch(m){
		case GET_TOKENS:
	...
	public void doMessage(CommandSender player, Error r, String args){
		String s = null;
		switch(r){
		case INCORRECT_SYNTAX:
			s = ChatColor.translateAlternateColorCodes('&', p.getConfig().getString("Messages.SyntaxError", "&4Error: [Incorrect Syntax] Options: [/dc]"));
	...
	public void doMessage(CommandSender player, Success r, String name, String num){
		String s = null;
		switch(r){
		case DP_RESTART:
			s = ChatColor.translateAlternateColorCodes('&', p.getConfig().getString("Messages.DPRestart", "&7You have restarted the drop party!"));
